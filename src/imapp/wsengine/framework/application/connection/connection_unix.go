...
import (
	...
	nodeSerialize "ws/framework/application/constant/binary/serialize"
...
func (c *Connection) SetEventListener(listener containerInterface.ConnectionEventListener) {
	...
	_ = c.conn.AddCloseCallback(c.onConnectionCloseCallback)
	_ = c.conn.SetOnRequest(c.onRead)
}
...
func (c *Connection) createConnection() (netpoll.Connection, error) {
	...
	// 连接基础设置
	netpollConn := conn.(netpoll.Connection)
	_ = netpollConn.SetReadTimeout(networkConstant.TCPRWTimeout)
	_ = netpollConn.SetWriteTimeout(networkConstant.TCPRWTimeout)
	_ = netpollConn.SetIdleTimeout(networkConstant.TCPKeepAliveTime)

	return netpollConn, nil
}
...
func (c *Connection) onRead(_ context.Context, connection netpoll.Connection) error {
...
func (c *Connection) read(reader netpoll.Reader) error {
