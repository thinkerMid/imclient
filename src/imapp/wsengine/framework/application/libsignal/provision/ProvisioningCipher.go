...
import (
	...
	"ws/framework/application/libsignal/util/bytehelper"
...
func Decrypt(privateKey string, content string) (string, error) {
	ourPrivateKey, err := base64x.StdEncoding.DecodeString(privateKey)
	if err != nil {
		return "", err
	}
	envelopeDecode, err := base64x.StdEncoding.DecodeString(content)
	if err != nil {
		return "", err
	}
