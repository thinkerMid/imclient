...
import (
	...
	"ws/framework/application/libsignal/ratchet"
	...
	"ws/framework/application/libsignal/util/medium"
	"ws/framework/application/libsignal/util/optional"
)

// NewBuilder constructs a session builder.
func NewBuilder(sessionStore store.ISessionStore, preKeyStore store.PreKey,
...
func (b *Builder) Process(sessionRecord *record.Session, message *protocol.PreKeySignalMessage) (unsignedPreKeyID *optional.Uint32, err error) {
	...
	//dstIDPublicKey := message.IdentityKey().PublicKey().PublicKey()
	//cacheIDPublicKey, err := b.identityKeyStore.FindIdentity(b.remoteAddress)
	//
	//if err != nil {
	//	b.identityKeyStore.CreateIdentity(b.remoteAddress, dstIDPublicKey[:])
	//} else if bytes.Compare(cacheIDPublicKey, dstIDPublicKey[:]) != 0 {
...
func (b *Builder) processV3(sessionRecord *record.Session,
	...
	ourSignedPreKeyRecord := b.signedPreKeyStore.FindSignedPreKey(message.SignedPreKeyID())
	...
	if !message.PreKeyID().IsEmpty {
		oneTimePreKey := b.preKeyStore.FindPreKey(message.PreKeyID().Value)
...
func (b *Builder) ProcessBundle(preKey *prekey.Bundle) error {
	...
	//dstIDPublicKey := preKeyPublic.PublicKey()
	//cacheIDPublicKey, err := b.identityKeyStore.FindIdentity(b.remoteAddress)
	//
	//if err != nil {
	//	b.identityKeyStore.CreateIdentity(b.remoteAddress, dstIDPublicKey[:])
	...
	b.sessionStore.SaveSession(b.remoteAddress, sessionRecord)
