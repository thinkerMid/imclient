...
type WamEventWamTestAnonymous0 struct {
	WAMessageEvent
...
func (event *WamEventWamTestAnonymous0) InitFields(option interface{}) {
	event.PsTestEnumField = 1
	event.PsTestFloatField = utils.LogRandSecond(1*time.Second, 2*time.Second)
}

func (event *WamEventWamTestAnonymous0) Serialize(buffer eventSerialize.IEventBuffer) {
	buffer.Header().
