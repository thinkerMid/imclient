...
func (r *binaryDecoder) readPacked8(tag byte) (string, error) {
...
func unpackByte(tag byte, value byte) (byte, error) {
	switch tag {
	case token.Nibble8:
...
func (r *binaryDecoder) readListSize(tag byte) (int, error) {
	switch tag {
...
func (r *binaryDecoder) read(string bool) (interface{}, error) {
	...
	case token.Dictionary0, token.Dictionary1, token.Dictionary2, token.Dictionary3:
		...
		return token.GetDoubleToken(int(tag-token.Dictionary0), i)
	case token.JIDPair:
	...
	default:
		if tag >= 1 && int(tag) < len(token.SingleByteTokens) {
			return token.SingleByteTokens[tag], nil
		}
		return "", fmt.Errorf("%w %d at position %d", waBinary.ErrInvalidToken, tag, r.index)
	}
}
...
func (r *binaryDecoder) readJIDPair() (interface{}, error) {
	...
	if err != nil {
		...
		return nil, waBinary.ErrInvalidJIDType
	} else if user == nil {
...
func (r *binaryDecoder) readAttributes(n int) (waBinary.Attrs, error) {
	if n == 0 {
		return nil, nil
	}

	ret := make(waBinary.Attrs)
	for i := 0; i < n; i++ {
		...
		if !ok {
			return nil, fmt.Errorf("%[1]w at position %[3]d (%[2]T): %+[2]v", waBinary.ErrNonStringKey, key, r.index)
		}
...
func (r *binaryDecoder) readList(tag byte) ([]waBinary.Node, error) {
	size, err := r.readListSize(tag)
	if err != nil {
		return nil, err
	}

	ret := make([]waBinary.Node, size)
	for i := 0; i < size; i++ {
		n, err := r.readNode()
...
func (r *binaryDecoder) readNode() (*waBinary.Node, error) {
	ret := &waBinary.Node{}
	...
	listSize, err := r.readListSize(byte(size))
	if err != nil {
		return nil, err
	}
	...
	if listSize == 0 || ret.Tag == "" {
		return nil, waBinary.ErrInvalidNode
	}
...
func (r *binaryDecoder) readRaw(length int) ([]byte, error) {
	...
	copy(ret, r.data[r.index:r.index+length])
	r.index += length
