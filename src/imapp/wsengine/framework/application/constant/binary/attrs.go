...
func (au *AttrUtility) GetJID(key string, require bool) (jidVal types.JID, ok bool) {
	...
	if val, ok = au.Attrs[key]; !ok {
		if require {
			au.Errors = append(au.Errors, fmt.Errorf("didn't find required UserID attribute '%s'", key))
		}
	} else if jidVal, ok = val.(types.JID); !ok {
		au.Errors = append(au.Errors, fmt.Errorf("expected attribute '%s' to be UserID, but was %T", key, val))
	}
	return
}
