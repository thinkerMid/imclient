...
import (
	...
	"labs/src/imapp/wsa/types/helpers"
)
...
func NewSignedPreKeyFromStruct(structure *SignedPreKeyStructure,
	...
	signedPreKey := &SignedPreKey{
		...
		signature:  helpers.SliceToArray64(structure.Signature),
	}

	// Generate the ECC key from bytes.
	publicKey := ecc.NewDjbECPublicKey(helpers.SliceToArray(structure.PublicKey))
	privateKey := ecc.NewDjbECPrivateKey(helpers.SliceToArray(structure.PrivateKey))
...
func NewSignedPreKey(id uint32, timestamp int64, keyPair *ecc.ECKeyPair,
	...
	return &SignedPreKey{
		structure: SignedPreKeyStructure{
			...
			PrivateKey: helpers.ArrayToSlice(keyPair.PrivateKey().Serialize()),
			Signature:  helpers.ArrayToSlice64(sig),
		},
		keyPair:    keyPair,
		signature:  sig,
		serializer: serializer,
